package integrate_probable_annotation::integrate_probable_annotationServer;
...
our %return_counts = (
        'integrate_annotation' => 1,
        'integrate_annotation_async' => 1,
        'integrate_annotation_check' => 1,
        'version' => 1,
);

our %method_authentication = (
        'integrate_annotation' => 'required',
        'integrate_annotation_async' => 'required',
        'integrate_annotation_check' => 'required',
);
...
our %async_run_methods = (
        'integrate_annotation_async' => 'integrate_probable_annotation.integrate_annotation',
);

our %async_check_methods = (
        'integrate_annotation_check' => 'integrate_probable_annotation.integrate_annotation',
);
...
sub get_service_name
{
    ...
    if(!defined $ENV{$SERVICE}) {
        return 'integrate_probable_annotation';
    }
    return $ENV{$SERVICE};
}
...
sub call_method {
    ...
    my $ctx = integrate_probable_annotation::integrate_probable_annotationServerContext->new($self->{loggers}->{userlog},
...
{
    # Service integrate_probable_annotation requires authentication.
	...
	{
	    $self->exception('PerlError', "Authentication required for integrate_probable_annotation but no authentication header was passed");
	}
    ...
    my @result;
    {
        ...
        my $stderr = integrate_probable_annotation::integrate_probable_annotationServerStderrWrapper->new($ctx, $get_time);
        $ctx->stderr($stderr);
...
sub get_method
{
    ...
    {
	$self->exception('NoSuchMethod',
			 "'$method' is not a valid method in service integrate_probable_annotation.");
    }
...
package integrate_probable_annotation::integrate_probable_annotationServerContext;

use strict;

=head1 NAME

integrate_probable_annotation::integrate_probable_annotationServerContext
...
package integrate_probable_annotation::integrate_probable_annotationServerStderrWrapper;
...
unless (caller) {
    ...
    my $server = integrate_probable_annotation::integrate_probable_annotationServer->new(
