...
my $config = new Config::Simple($config_file)->get_block('integrate_probable_annotation');
my $ws_url = $config->{"workspace-url"};
my $ws_name = undef;
my $ws_client = new Bio::KBase::workspace::Client($ws_url,token => $token);
my $auth_token = Bio::KBase::AuthToken->new(token => $token, ignore_authrc => 1);
my $ctx = LocalCallContext->new($token, $auth_token->user_id);
$integrate_probable_annotation::integrate_probable_annotationServer::CallContext = $ctx;
my $impl = new integrate_probable_annotation::integrate_probable_annotationImpl();
=head
sub get_ws_name {
    if (!defined($ws_name)) {
        my $suffix = int(time * 1000);
        $ws_name = 'test_integrate_probable_annotation_' . $suffix;
...
eval {
my $ret = $impl->integrate_annotation($ws, $genome, $mR, $outPut);
 #print &Dumper ($ret);
};
